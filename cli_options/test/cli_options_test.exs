defmodule CliOptionsTest do
  ...
  describe "parse/2" do
    ...
    test "multiple with default set" do
      ...
      # `default` does not have to be a list if `multiple` is true
      schema = [file: [type: :string, multiple: true, default: "foo.ex"]]

      assert {:ok, {[file: ["foo.ex"]], [], []}} = CliOptions.parse([], schema)
