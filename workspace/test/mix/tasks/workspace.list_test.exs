defmodule Mix.Tasks.Workspace.ListTest do
  ...
  @tag :tmp_dir
  test "with --format json and --show-status", %{tmp_dir: tmp_dir} do
    Workspace.Test.with_workspace(
      tmp_dir,
      [],
      :default,
      fn ->
        ...
        assert length(data["projects"]) == 11
        # The JSON output should include status information when --show-status is used
        # This is handled by the Workspace.Export.to_json function
      end,
      git: true
    )
  end
  ...
  @tag :tmp_dir
  test "with --format json and --base/--head options", %{tmp_dir: tmp_dir} do
    Workspace.Test.with_workspace(
      tmp_dir,
      [],
      :default,
      fn ->
        ...
        assert length(data["projects"]) == 11
        # The JSON output should include status information when --base/--head are used
      end,
      git: true
    )
  end
  ...
  @tag :tmp_dir
  test "with --format json and --maintainer filtering", %{tmp_dir: tmp_dir} do
    Workspace.Test.with_workspace(
      tmp_dir,
      [],
      :default,
      fn ->
        ...
        assert length(data["projects"]) == 1
        assert hd(data["projects"])["app"] == "package_a"
      end
    )
  end

  @tag :tmp_dir
  test "with --format json and --dependency filtering", %{tmp_dir: tmp_dir} do
    Workspace.Test.with_workspace(
      tmp_dir,
      [],
      :default,
      fn ->
        ...
        assert length(data["projects"]) == 2
        project_names = Enum.map(data["projects"], & &1["app"])
        assert "package_a" in project_names
        assert "package_h" in project_names
      end
    )
  end

  @tag :tmp_dir
  test "with --format json and --dependent filtering", %{tmp_dir: tmp_dir} do
    Workspace.Test.with_workspace(
      tmp_dir,
      [],
      :default,
      fn ->
        ...
        assert length(data["projects"]) == 3
        project_names = Enum.map(data["projects"], & &1["app"])
        assert "package_b" in project_names
        assert "package_c" in project_names
        assert "package_d" in project_names
      end
    )
  end

  @tag :tmp_dir
  test "with --format json and empty workspace", %{tmp_dir: tmp_dir} do
    Workspace.Test.with_workspace(tmp_dir, [], [], fn ->
      output = capture_io(fn ->
        ListTask.run([
          "--workspace-path",
      ...
      assert length(data["projects"]) == 0
      assert data["workspace_path"] == tmp_dir
    end)
  end

  @tag :tmp_dir
  test "with --format json and --project filtering", %{tmp_dir: tmp_dir} do
    Workspace.Test.with_workspace(
      tmp_dir,
      [],
      :default,
  ...
  @tag :tmp_dir
  test "with --format json and --path filtering", %{tmp_dir: tmp_dir} do
    Workspace.Test.with_workspace(
      tmp_dir,
      [],
      :default,
  ...
  @tag :tmp_dir
  test "with --format json and --tags filtering", %{tmp_dir: tmp_dir} do
    Workspace.Test.with_workspace(
      tmp_dir,
      [],
      :default,
